/*
Напишите логику работы с лицевым счетом в банке. У счета есть баланс (сумма на счете) и овердрафт (на какую сумму можно уйти в минус).
К счету последовательно применяются транзакции списания и пополнения. Транзакции изменяют баланс счета, овердрафт не меняется.
...
func (acc *account) deposit(amount int) error {
	...
	if amount > 0 {
		...
		return errors.New("amount is not valid, expect amount > 0")
	}
}
...
func (acc *account) withdraw(amount int) error {
	...
	if acc.balance+acc.overdraft < amount || acc.balance+acc.overdraft-amount < 0 {
		return errors.New("amount is not valid, expect amount < balance+overdraft")
	}
	acc.balance -= amount
	return nil
}
